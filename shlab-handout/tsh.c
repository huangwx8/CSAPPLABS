...
int fg_status = 0;          /* fore ground job terminated by a SIGINT, shell should print relative message */
...
void eval(char *cmdline)
{
    ...
    if (!builtin_cmd(argv)) {
		...
		/* add new job infomation into global variable "jobs" */
...
void sigint_handler(int sig)
{
	/* keyboard interupt send a signal to shell process, shell should
	transmit this singal to fg process */
...
/* jid2pid - Map process job ID to ID */
int jid2pid(int jid)
{
	int i;
